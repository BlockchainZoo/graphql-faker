...
function arrayResolver(itemResolver, _a) {
    var sample = _a.sample;
    var options = __assign({ min: 2, max: 4 }, sample);
    ...
    return function () {
        ...
        var length = fake_1.getRandomInt(options.min, options.max);
        var result = [];
        while (length-- !== 0)
...
function getFakeDirectives(object) {
    ...
    if (directives.isApplied('sample'))
        result.sample = directives.getDirectiveArgs('sample');
    return result;
}
